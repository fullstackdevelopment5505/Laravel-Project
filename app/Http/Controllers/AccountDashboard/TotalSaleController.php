...
class TotalSaleController extends Controller
{
    ...
    public function TotalSaleView()
    {
        ...
        return view('AccountDashboard.sale.totalSaleReport');
    }
    ...
    function TotalSaleList(Request $request){
        ...
        DB::raw('(CASE WHEN invoices.status="Credit" THEN "TC-07" END) as taxCode'),
        DB::raw('(CASE WHEN invoices.status="Credit" THEN "Invoice " END) as type'),
        DB::raw('(CASE WHEN invoices.status="Credit" THEN "SGL-07" END) as glCode'));

        if($request->date_from != '' && $request->date_to !=''){
            // echo 'HERE';
             $invoice_sale_start_date_from = date('Y-m-d', strtotime(str_replace('-', '/', $request->get('date_from'))));
             $invoice_sale_start_date_to = date('Y-m-d', strtotime(str_replace('-', '/', $request->get('date_to'))));
             $InvoiceSale->whereBetween('invoices.date',array($invoice_sale_start_date_from. ' 00:00:00', $invoice_sale_start_date_to . ' 23:59:59'));
         }
         if($request->date_from == '' && $request->date_to !=''){
             //echo 'HERE2';
             $invoice_sale_start_date_to = date('Y-m-d', strtotime(str_replace('-', '/', $request->get('date_to'))));
             $InvoiceSale->where('invoices.date','<=',$invoice_sale_start_date_to . ' 23:59:59');
         }
         if($request->date_from != '' && $request->date_to ==''){
             //echo 'HERE3';
             $invoice_sale_start_date_from = date('Y-m-d', strtotime(str_replace('-', '/', $request->get('date_from'))));
             $InvoiceSale->where('invoices.date', '>=',$invoice_sale_start_date_from . ' 00:00:00');
         }

         $res4 = $InvoiceSale->where('status','credit')->get();
        ...
        DB::raw('((tbl_deposite.amount)*20)/100 as tax'),DB::raw('((tbl_deposite.amount)*80)/100 as cash'), DB::raw('(CASE WHEN tbl_membership.membership_type="1" THEN "TC-02" END) as taxCode'),
