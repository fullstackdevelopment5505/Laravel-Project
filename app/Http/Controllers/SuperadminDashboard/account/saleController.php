...
class saleController extends Controller
{
    ...
    {
        return view('SuperadminDashboard.sale.salePropertyReport');
    }
    ...
    function saleReportList(Request $request){
        if($request->ajax())
        {
            $abc = 0;
                ...
                $data = DB::table('tbl_purchased_records')->select('tbl_purchased_records.user_id','tbl_purchased_records.report_name AS property_type','tbl_purchased_records.created_at','points_transaction.amount',DB::raw('((points_transaction.amount)*20)/100 as tax'),DB::raw('((points_transaction.amount)*80)/100 as cash'),DB::raw('(CASE WHEN user_detail.f_name <> "" THEN user_detail.f_name ELSE SUBSTRING_INDEX(users.email, "@", 1) END) as name'),DB::raw('((points_transaction.amount)*0) as total'))
                ...
                $data = $data->orderBy('tbl_purchased_records.id','asc')->get();
        ...
        ->Join('user_detail', 'users.id', '=', 'user_detail.user_id')
        ->Join('datatree', 'tbl_purchased_records.property_id', '=', 'datatree.PropertyId')
        ->Join('points_transaction', 'tbl_purchased_records.point_id', '=', 'points_transaction.id')->orderBy('tbl_purchased_records.created_at','desc')->get();
    ...
    {
        return view('SuperadminDashboard.sale.saleMembershipReport');
    }
