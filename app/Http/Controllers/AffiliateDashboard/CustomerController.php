...
class CustomerController extends Controller
{
    ...
    public function MemberDetail(Request $request,$id,$search_type){
	   ...
	   //report purchased
        ...
        return view('AffiliateDashboard/member/memberDetail',compact('report_purchased_count','sale_executiveName','sale_managerName','member','expiry_date','daysLeft','search_type','data','highly_interested_properties', 'progressBar' ,'deposite'));
    ...
    public function memberSearch(Request $request)
    {
        ...
        if(!empty($request->all()))
        {
            ...
            return view('AffiliateDashboard/member/memberSearchResult',compact('customers','search_type'));
         ...
         if($request->ajax())
        {
            ...
            if($request->get('type') == "transaction_record_credit"){
                ...
                $data = DB::table('points_transaction')->select('points_transaction.created_at AS date','points_transaction.point',
                    ...
                    ->leftJoin('users', 'points_transaction.user_id', '=', 'users.id')
                    ->leftJoin('tbl_deposite', 'points_transaction.trans_id', '=', 'tbl_deposite.id')
                    ->where('points_transaction.user_id',$request->get('user_id') )->where('points_transaction.type',1) -> where('tbl_deposite.balance_transaction', '!=','')-> orWhereNull('tbl_deposite.balance_transaction');//punch4
			...
			if($request->get('type') == "#interested_properties"){
                ...
                return view('AffiliateDashboard/member/propertiesResult', compact('data'));   
            ...
            if($request->get('type') == "#highly_interested_properties"){
               ...
               return view('AffiliateDashboard/member/highPropertiesResult', compact('highly_interested_properties'));   
