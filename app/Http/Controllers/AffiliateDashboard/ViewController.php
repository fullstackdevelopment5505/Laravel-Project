...
class ViewController extends Controller
{
	...
	public static function data($user_id,$type=''){
		
		if($type == 'top_executive_sale'){
			...
			'tbl_purchased_records.report_name','datatree.SitusStreetAddress','tbl_purchased_records.property_id',
			'datatree.AssdTotalValue','tbl_purchased_records.created_at')
			->groupBy('tbl_purchased_records.user_prop_id') 
			->orderBy('count', 'DESC')
			->where('users.mapped_to_affiliate',$user_id)
			->take(5)
			->get();
			return $topExecutiveSale;
		}
	}	
    public function dashboard(Request $request){
  
		$userId=Auth::guard('affiliate')->user()->id;
        ...
        ->where('users.mapped_to_affiliate',$userId)
        ->groupBy('year', 'month')
   ...
   if($currentMonthSale==""){
	...
	// $cPercentage=$changePercentage/100;
        ...
        ->where(['user_property.status'=>'2','users.mapped_to_affiliate'=>$userId])
        ->groupBy('property_id')
        // ->having('total', '>' , 1)
        ->take(3)
        ->get();
        $image = ['assets/affiliate/images/pro1.png','assets/superadmin/images/house1.jpg','assets/superadmin/images/house4.jpg'];
		...
		$customerJoinedLastMonth = $this->totalMember($userId,'last_month');
		
		$customerEntrolledLastMonth = $this->totalNonMember($userId,'last_month');
	   
        $customerJoined 	= $this->totalMember($userId);
		
		$customerEntrolled 	= $this->totalNonMember($userId);  
		...
		$topExecutiveSale 	= $this->data($userId,'top_executive_sale');  
        
       
		
        return view('AffiliateDashboard.dashboard')->with(['customerEnrolled'=>$customerEntrolled,
    ...
    public function properties(){
        ...
        return view('AffiliateDashboard.properties')->with(['interestedProperty'=>$interestedProperty,
	...
	if($earnedThisMonth==""){
	 ...
	 $earnedThisMonthSale=0;
        ...
        return view('AffiliateDashboard.sale')->with(['earnedTodaySale'=>$earnedTodaySale,
    ...
    public function customer(){
        ...
        return view('AffiliateDashboard.member.member')->with(['cities'=>$cities]);
    }
    
    public function team(){
        return view('AffiliateDashboard.team.team');
    }
    public function propertyDetail($id){
        ...
        ->first();
        return view('AffiliateDashboard.propertyDetail')->with('propertyDetail',$propertyDetail);
    }
    public function addTeam(){
        $state=States::select("*")->where('ID','5')->get();
        $city=DB::table('us_cities')->select("*")->where('ID_STATE','5')->get();
        return view('AffiliateDashboard.team.addTeam')->with(['state'=>$state,'city'=>$city]);
    }
    public function memberList(){
        return view('AffiliateDashboard.membersList');
    }
    public function nonMemberList(){
        return view('AffiliateDashboard.nonMemberList');
    }
    ...
    //    }
      ...
      ->LeftJoin('datatree', function($query){
       ...
       return view('AffiliateDashboard/membersDetail',compact('sale_executiveName',
    ...
    public function nonMemberDetail(Request $request,$id){
       ...
       ->LeftJoin('datatree', function($query){
        ...
        return view('AffiliateDashboard/nonMemberDetail',compact(
        'sale_managerName','member','data','highly_interested_properties'));
    }
    public function messages(){
        return view('AffiliateDashboard.message.message');
    }
    public function sendMessage(){
        return view('AffiliateDashboard.message.sendMessage');
    }
    public function viewMessage(){
        return view('AffiliateDashboard.message.viewMessage');
    }
	public function profile(){
		
		$userId=Auth::guard('affiliate')->user()->id;
		$data = AffiliateUsers::find($userId);
        ...
        return view('AffiliateDashboard.profile')->with(['url'=>Config::get('app.website_url'),'email'=>$data->email,'username'=>$data->username,'f_name'=>$data->full_name,
        'address'=>$data->address,'state'=>$data->state,'city'=>$data->city,'zipcode'=>$data->zipcode,'phone'=>$data->phone,'country'=>'US']);
    }
}
