        		...
        		<div class="alert alert-success">
					...
					@endif
						...
						<div class="col-sm-12 top_selling">
							<div class="inside">
								<div class="title">Purchase Data Finder</div>
								<table class="display responsive nowrap" width="100%" id="purchaseDatafinder">
									<thead>
										<tr>
										    <th>Date</th>
										    <th>Report Type</th>
			...
			<div class="modal-content">
				...
				<div class="modal-body register_new_user">
				  <form action="{{route('saveDataTreeFinderData')}}" method="post" id="customerForm" >
					...
					<input type="hidden" name="type" value="datafinder_single_entry">
			...
			<div class="modal-content">
				...
				<div class="modal-body register_new_user">
				  <form action="{{route('saveDataTreeFinderData')}}" method="post" id="customerForm1" >
					...
					<div class="col-md-12 text-center p-0"> 
					<input type="submit" class="btn btn-success svbtn" value="Save">
					<input type="hidden" name="type" value="datafinder_bulk_entry">
						<a href="#" type="btn" class="btn btn-secondary ml-1" data-dismiss="modal">Cancel</a>
					</div>
				</div>
				</form>				
...
<script type="text/javascript">	
	$(document).ready(function() {
		...
		$('#purchaseDatafinder').DataTable({
			"processing": true,
			"serverSide": true,
			"ajax": {
				url: "{{route('superadminPurchaseDataFinder')}}",
				data: function (d) {
					d.date_from_purchase = $('input[name="date_from"]').val();
					d.date_to_purchase =$('input[name="date_to"]').val();
				}
			},			
			"columns":[
				// {"data": 'DT_RowIndex'},
		...
		$('#search_submit_purchase').click(function(){	
			$('#purchaseDatafinder').DataTable().draw(true);
		});
	});
</script>
<script>
