                ...
                <div class="col-sm-12 top_bar_area">
					<div class="row">
                        ...
                        <div class="col-sm-4 top_btns">
                           ...
                           <?php /* <a href="send-message.php?active=custom" class="btn btn-success">Message <i class="fa fa-envelope"></i></a> */ ?>
							...
							<div class="status_button_wrap">
								<a data-id="{{$member->user_primary_id}}" href="javascript:void(0);" class="btn btn-danger deleteUser">Delete <i class="fa fa-trash"></i></a>
                            </div>
                        </div>
					</div>
				</div>
							...
							<div class="inside">
                                <div class="row">
                                    <div class="col-sm-12 all_customer_data">
										...
										<?php if($member->filename != ""){ ?> 
                                            <img src="{{asset($member->filename)}}">
										<?php }else{ ?>
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if($.isEmptyObject(data.error)){
                        ...
                        swal(data.success,"user deleted successfully", "success"); 
                        goBack();
   ...
   $('[data-toggle="pill"]').click(function(e) {
    ...
    function getData(page,href) {
        if(href == "#interested_properties"){
            ...
            $.get("{{route('superadminSavedSearchList')}}?page="+ page,
            {
    ...
    var table = $('#purchase_record').DataTable({
		...
		ajax: {
			...
			data: function (d) {
					...
					d.search_type = <?php echo $search_type; ?>;
					
				}
			},
			
			columns: [ 
				{ data: 'purchase_group_name', name: 'purchase_group_name'},
    ...
    var table = $('#table_trans_credit').DataTable({
        ...
        { data: 'txn', name: 'tbl_deposite.balance_transaction' },
        { data: 'amount', name: 'tbl_deposite.amount' },
      //  { data: 'point', name: 'point' },
        { data: 'payment_method', name: 'tbl_deposite.brand' }
        // {data: 'action', name: 'action', orderable: false, searchable: false}
        ]
    });
    ...
    var table = $('#table_trans_debit').DataTable({
        ...
        { data: 'date', name: 'date', format: 'd-M-yy', },
        { data: 'point', name: 'point' },
        { data: 'debit_type', name: 'tbl_purchased_records.report_name' }
        // {data: 'action', name: 'action', orderable: false, searchable: false}
        ]
    });
	...
	$('body').on('click', '.view_save_search_button', function(e) {
		...
		$.ajax({
			...
			success: function(response){
				...
				$('#showSearchBody').html(response.data);
				//$("#showSearchBody").find('#dyna').addClass('display responsive nowrap');
			}       
		});
    });
	
	$('body').on('click', '.purchase_detail_button', function(e) {
		e.preventDefault();
		console.log($(this).data('user-id'));
		console.log($(this).data('purchase-group-name'));
		var user_id = $(this).data('user-id');
		var member = $(this).data('member');
		var purchase_group_name = $(this).data('purchase-group-name');
		var type = 'purchase_property_detail';
		 $.ajax({
			type: "GET",
			url: "{{route('superadminSavedSearchList')}}",
			data: { user_id: user_id, type: type,purchase_group_name:purchase_group_name,member:member },
			success: function(data){
				if(data.message=='success'){
					window.location.replace('/superadmin/member/detail/purchasedetail/'+data.userid+'/'+data.name+'/'+data.member);
